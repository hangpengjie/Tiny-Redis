...
// key -> val
static int32_t do_get( std::vector<std::string> &cmd, uint8_t *res, uint32_t *reslen){
    // get key
    Entry key;
    key.key.swap(cmd[1]);
...
static int32_t do_set(std::vector<std::string> &cmd, uint8_t *res, uint32_t *reslen){
...
static int32_t do_del( std::vector<std::string> &cmd, uint8_t *res, uint32_t *reslen){
    Entry key;
    key.key.swap(cmd[1]);
    key.node.hcode = str_hash((uint8_t *)key.key.data(), key.key.size());
